...
const Home = () => {
  ...
  const averageRating = (
    movies.reduce((acc, movie) => acc + movie.rating, 0) / totalMovies
  ).toFixed(1);
  return (
    <div className="p-8 space-y-8 bg-gray-900 min-h-screen">
      ...
      <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {movies.map((movie) => (
          <li
            key={movie.id}
            className="bg-gray-800 p-6 rounded-lg shadow-xl hover:shadow-2xl transition-all duration-300 ease-in-out"
          >
            <Link
